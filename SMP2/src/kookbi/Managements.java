	...
	public void view () {
		...
		do {
			...
			try {
				...
				System.out.println("종료합니다. 숫자를 1~8까지만 입력해주세요.");
				break;
			}
			sc.nextLine();
	...
	void search () {
		...
		while(iter.hasNext()) {
			...
			if(name.equals(std.getName())) {
				std.show();
				System.out.printf("| %6s    |  %6s    |  %6s  |  %5s      |\n", arSubjects[0], arSubjects[1], arSubjects[2], arSubjects[3]);
			...
			if (num == std.getNumber()) {
				...
				case 2: 
					...
					System.out.println("[점수 수정]"
							+ "\n각 과목의 점수를 순서대로 입력하세요. "
							+ "\n(C언어 JAVA Spring)");
					for (int i = 0; i < SUBJECTS; i++) {
						int temp = sc.nextInt();
						if (temp > 100 || temp < 0) {
							System.out.println("0~100점 사이가 아닌 과목이 있습니다. 해당 과목을 제외하고 수정합니다.");
							continue;
						}else {
							StudentsBook.get(std).set(i, temp);
						}
					}
